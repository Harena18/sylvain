    ...
    const handleDeleteItem = async (id) => {
        ...
        try {
            const res = await fetch(
                `http://localhost/backend/items/items.php?id=${encodeURIComponent(
                    id
                )}`,
                {
                    method: "DELETE",
    ...
    return (
        <>
            <div className="px-5 py-12 rounded-2xl min-h-full">
                ...
                <div className="text-center mb-5 grid grid-cols-12">
                    ...
                    {clothes.map((clothe) => (
                        <React.Fragment key={clothe.id}>
                            <div className="col-span-1 border-y p-2">
                                {clothe.id}
                            </div>
                            <div className="col-span-5 border-y p-2">
                                {clothe.name}
                            </div>
                            <div className="col-span-4 border-y p-2">
                                {clothe.size}
                            </div>
                ...
                <div className="text-center mb-5 grid grid-cols-12">
                    ...
                    {display.map((d) => (
                        <React.Fragment key={d.id}>
                            ...
                            <div className="col-span-7 border-y p-2">
                                {d.name}
                            </div>
                            <div className="col-span-2 border-y p-2">
                                {d.format}
                ...
                {active ? (
                    <Create setActive={setActive} setRefresh={setRefresh} />
                ) : (
                    ""
                )}
            </div>
