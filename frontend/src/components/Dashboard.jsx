...
function Dashboard() {
  ...
  return (
    <div className="px-5 py-12 rounded-2xl min-h-full">
      <div className="text-center font-bold text-2xl">Tableau de bord</div>
        ...
        <table className="min-w-full shadow-md rounded-lg overflow-hidden">
          ...
          <tbody>
            {personalisations.map((p) => (
              <tr key={p.id} className="border-b hover:bg-gray-50/10">
                <td className="py-2 px-5">{p.client_nom || p.client_id}</td>
                <td className="py-2 px-5">{p.item_nom || p.item_id}</td>
                <td className="py-2 px-5">{p.motif}</td>
                <td className="py-2 px-5">{p.date_debut?.slice(0, 10)}</td>
                <td className="py-2 px-5">{p.date_fin?.slice(0, 10) || "-"}</td>
                <td className="py-2 px-5">{p.nombre_exemplaires}</td>
