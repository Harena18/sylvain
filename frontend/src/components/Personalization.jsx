...
function Personalization() {
  ...
  useEffect(() => {
    fetch("http://localhost:8000/personalization/personalization.php")
      .then((res) => res.json())
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await fetch(
        "http://localhost:8000/personalization/personalization.php",
        {
          method: "POST",
  ...
  return (
    <div className="p-4">
      <h2 className="text-center font-bold text-2xl">Personnalisations</h2>

      {/* En-têtes */}
      <div className="my-4 grid grid-cols-8">
        <div className="col-span-1 border-b-2 p-2 font-bold">Client</div>
        <div className="col-span-1 border-b-2 p-2 font-bold">Objet</div>
        <div className="col-span-1 border-b-2 p-2 font-bold">Motif</div>
        <div className="col-span-1 border-b-2 p-2 font-bold">Quantité</div>
        <div className="col-span-1 border-b-2 p-2 font-bold">Début</div>
        <div className="col-span-1 border-b-2 p-2 font-bold">Fin</div>
        <div className="col-span-1 border-b-2 p-2 font-bold">Statut</div>
        <div className="col-span-1 border-b-2 p-2 font-bold">Actions</div>
        ...
        {personalisations.map((p) => (
          <React.Fragment key={p.id}>
            ...
            <div className="col-span-1 border-b-2 p-2">
              {p.item_nom ? `${p.item_nom} (id:${p.item_id})` : p.item_id}
            </div>
            ...
            <div className="col-span-1 border-b-2 p-2">
              <input
                ...
                onChange={(e) => setEditField(p.id, "motif", e.target.value)}
              />
            </div>
            ...
            <div className="col-span-1 border-b-2 p-2">{p.date_debut}</div>
            ...
            <div className="col-span-1 border-b-2 p-2">
              <input
                ...
                onChange={(e) => setEditField(p.id, "date_fin", e.target.value)}
              />
            </div>
            ...
            <div className="col-span-1 border-b-2 p-2">
              <select
                ...
                onChange={(e) => setEditField(p.id, "statut", e.target.value)}
              >
            ...
            <div className="col-span-1 border-b-2 p-2 flex gap-2">
              <button
                onClick={() =>
                  handleUpdate(p.id, {
                    motif: getBuffered(p.id, "motif", p.motif),
                    date_fin: getBuffered(
                      p.id,
                      "date_fin",
                      (p.date_fin || "").slice(0, 10)
                    ),
                    statut: getBuffered(p.id, "statut", p.statut),
                  })
                }
      ...
      <div className="mt-30 border rounded-2xl py-10 px-5">
        ...
        <form onSubmit={handleSubmit} className="flex flex-col gap-2 mt-2">
          <input
            ...
            onChange={(e) =>
              setNewPerso({ ...newPerso, client_id: e.target.value })
            }
            required
            className="p-2"
          />
          <input
            ...
            onChange={(e) =>
              setNewPerso({ ...newPerso, item_id: e.target.value })
            }
            required
            className="p-2"
          />
          ...
          <input
            ...
            onChange={(e) =>
              setNewPerso({ ...newPerso, date_fin: e.target.value })
            }
            className="p-2"
          />
          <input
            ...
            onChange={(e) =>
              setNewPerso({ ...newPerso, nombre_exemplaires: e.target.value })
            }
            className="p-2"
          />
